<?php include "templates/include/header.php" ?>
	...
	<div id="AddDeleteButtons" class="fontStyle">
        <?php if ( $results['dailyMenuItem']->id) { ?>
            <p><a class="Links" href="index.php?action=deleteDailyMenuItem&amp;dailyMenuItemId=<?php echo $results['dailyMenuItem']->id ?>" onclick="return confirm('Delete this item')">Delete this item</a> </p>
        <?php } ?>
	</div>
	<div id="div-20">
	...
	<div id="MainContent" class="fontStyle">
        ...
        <input type="hidden" name="dailyMenuItemId" value="<?php echo $results['dailyMenuItem']->id ?>"/>
        <input type="hidden" name="date" value="<?php echo date( "Y-m-d")?>"/>
        <ul>
            <li>
                <label for="name">Name</label>
                <input type="text" name="name" id="name" placeholder="Name of the food item" required autofocus maxlength="50" value="<?php echo htmlspecialchars( $results['dailyMenuItem']->name )?>" />
            </li>
            <li>
                <label for="price">Price</label>
                <input type="text" name="price" id="price" placeholder="Item price" required maxlength="2" value="<?php echo $results['dailyMenuItem']->price ?>"/>
            </li>
            <li>
                <label for="description">Description</label>
                <textarea name="description" id="description" placeholder="Description of the food item" required maxlength="100" style="vertical-align:top;" rows="2" cols="50"><?php echo htmlspecialchars( $results['dailyMenuItem']->description )?></textarea>
            </li>
        </ul>
