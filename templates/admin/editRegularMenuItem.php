<?php include "templates/include/header.php" ?>
	...
	<div id="AddDeleteButtons" class="fontStyle">
        <?php if ( $results['regularMenuItem']->id) { ?>
            <p><a class="Links" href="index.php?action=deleteRegularMenuItem&amp;regularMenuItemId=<?php echo $results['regularMenuItem']->id ?>" onclick="return confirm('Delete this item')">Delete this item</a> </p>
        <?php } ?>
	</div>
	<div id="div-20">
	...
	<div id="MainContent" class="fontStyle">
        ...
        <input type="hidden" name="regularMenuItemId" value="<?php echo $results['regularMenuItem']->id ?>"/>
        <input type="hidden" name="date" value="<?php echo date( "Y-m-d")?>"/>
        <ul>
            <li>
                <label for="name">Name</label>
                <input type="text" name="name" id="name" placeholder="Name of the food item" required autofocus maxlength="50" value="<?php echo htmlspecialchars( $results['regularMenuItem']->name )?>" />
            </li>
            <li>
                <label for="price">Price</label>
                <input type="text" name="price" id="price" placeholder="Item price" required maxlength="2" value="<?php echo $results['regularMenuItem']->price ?>"/>
            </li>
            <li>
                <label for="price">Type</label>
                <select name="type" id="type" required>
                    <option value="soup_salad_appy">Soup,Salad&Appys</option>
                    <option value="main_forno">Main&Forno</option>
                    <option value="sandwich_pasta">Sandwich&Pastas</option>
                </select>
            </li>
            <li>
                <label for="description">Description</label>
                <textarea name="description" id="description" placeholder="Description of the food item" required maxlength="100" style="vertical-align:top;" rows="2" cols="50"><?php echo htmlspecialchars( $results['regularMenuItem']->description )?></textarea>
            </li>
        </ul>
