<?php include "templates/include/header.php" ?>
	...
	<div id="AddDeleteButtons" class="fontStyle">
        <?php if ( $results['eventItem']->id) { ?>
            <p><a class="Links" href="index.php?action=deleteEventItem&amp;eventItemId=<?php echo $results['eventItem']->id ?>" onclick="return confirm('Delete this item')">Delete this item</a> </p>
        <?php } ?>
	</div>
	<div id="div-20">
	...
	<div id="MainContent" class="fontStyle">
        ...
        <input type="hidden" name="eventItemId" value="<?php echo $results['eventItem']->id ?>"/>
        <input type="hidden" name="date" value="<?php echo date( "Y-m-d")?>"/>
        <ul>
            <li>
                <label for="name">Name</label>
                <input type="text" name="name" id="name" placeholder="Name of event" required autofocus maxlength="50" value="<?php echo htmlspecialchars( $results['eventItem']->name )?>" />
            </li>
            <li>
                <label for="description">Description</label>
                <textarea name="description" id="description" placeholder="Description of event" required maxlength="100" style="vertical-align:top;" rows="2" cols="50"><?php echo htmlspecialchars( $results['eventItem']->description )?></textarea>
            </li>
        </ul>
